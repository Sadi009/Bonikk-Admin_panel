...
function initMap() {
    ...
    var contentString = '<div class="info-window-content"><h2>bonikk Dashboard</h2>' +
...
$('#menuItems').on('click', '.dropdown-item', function(){
    $('#dropdown_items').text($(this)[0].value)
    $("#dropdown_items").dropdown('toggle');
})
...
function onAddCatagory(){
	(async () => {
		...
		const { value: formValues } = await Swal.fire({
		  ...
		  focusConfirm: false,
		  preConfirm: () => {
			return "New Catagoy is added in the list"
		  }
		})
...
// Sub Catagory Page 
// Add Sub-catagory Model 
function onAddSubCatagory(){
	(async () => {

		const { value: subCatagory } = await Swal.fire({
		  title: 'Add Sub Catagory',
		  input: 'text',
		  inputPlaceholder: 'Add Sub Catagory',
		  showCancelButton: true,
		  inputAttributes: {
			autocapitalize: 'off',
			autocorrect: 'off'
		...
		if (subCatagory) {
		  Swal.fire(` ${subCatagory} is added on sub catagory list`)
		}
